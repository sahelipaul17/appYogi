document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.on('initialKeyStates', (keyStates) => {
      keyStates.forEach((keyState) => {
        const { key, color } = keyState;
        const keyElement = keyboard.children[key];
    ...
    socket.on('controlAcquired', (socketId) => {
        ...
        console.log('Control granted to user:', socketId);
      ...
      socket.on('updateRemoteScreen', (data) => {
        ...
        const remoteKey = keyboard.children[key];
        remoteKey.style.backgroundColor = color;
      });
